...
//logout users out after 1 hour
	...
	$form = sanitize_sql_input($_GET["form"],20,"/[^a-zA-Z1-9 ]/");
	$track = sanitize_sql_input($_GET["track"],20,"/[^a-zA-Z1-9 ]/");
	$department= sanitize_sql_input($_GET["department"],20,"/[^a-zA-Z1-9 ]/");
	$class= sanitize_sql_input($_GET["class"],20,"/[^a-zA-Z1-9 ]/");
    $subject =sanitize_sql_input($_GET["subject"],20,"/[^a-zA-Z1-9 ]/");
    $exam=sanitize_sql_input($_GET["exam"],20,"/[^a-zA-Z1-9 ]/");
	...
	if(! $conn)
	{
		print("<script>alert('connection error');document.location.href='../../login/index.php'</script>");
		exit;
	}
